...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 grid = False, win_grid_size = 8, n_cores=4, tt_rank=8):
        super().__init__()
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.block1 = nn.ModuleList([
            Block(
                ...
                grid=False, win_grid_size=win_grid_size[0],
                n_cores=n_cores[0], tt_rank=tt_rank[0]
            ),
            Block(
                dim=embed_dims[0], num_heads=num_heads[0], mlp_ratio=mlp_ratios[0], qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.block2 = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + 0], norm_layer=norm_layer,
                grid=False, win_grid_size=win_grid_size[1],
                n_cores=n_cores[1], tt_rank=tt_rank[1]
            ),
            Block(
                dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + 1], norm_layer=norm_layer,
        ...
        self.block3 = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + 0], norm_layer=norm_layer,
                grid=False, win_grid_size=win_grid_size[2],
                n_cores=n_cores[2], tt_rank=tt_rank[2]
            ),
            Block(
                dim=embed_dims[2], num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + 1], norm_layer=norm_layer,
        ...
        self.block4 = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + 0], norm_layer=norm_layer,
                grid=False, win_grid_size=win_grid_size[3],
                n_cores=n_cores[3], tt_rank=tt_rank[3]
            ),
            Block(
                dim=embed_dims[3], num_heads=num_heads[3], mlp_ratio=mlp_ratios[3], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + 1], norm_layer=norm_layer,
...
@BACKBONES.register_module()
class b1_maxvit_tt_5cores_6ranks(MixVisionTransformer):
    def __init__(self, **kwargs):
        super(b1_maxvit_tt_5cores_6ranks, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], 
            win_grid_size=[8, 8, 8, 8], n_cores=[5, 5, 5, 5], tt_rank=[6, 6, 6, 6],
            drop_rate=0.0, drop_path_rate=0.1)


@BACKBONES.register_module()
class b1_maxvit_tt_5cores_6ranks_16winsize(MixVisionTransformer):
    def __init__(self, **kwargs):
        super(b1_maxvit_tt_5cores_6ranks_16winsize, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], 
            win_grid_size=[16, 16, 16, 16], n_cores=[5, 5, 5, 5], tt_rank=[6, 6, 6, 6],
            drop_rate=0.0, drop_path_rate=0.1)
